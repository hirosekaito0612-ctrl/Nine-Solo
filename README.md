...
- 😺 **キャラの反応が変わる**: 今日の本数に応じてヤメにゃんのセリフと画面エフェクトが変化
  ...
  | いいペース | 〜34% | ほっぺ |
  | ちょっと注意 | 〜75% | あせ💦 |
  | ひとやすみ | 76%〜 | なみだ＋くらいビネット |
- 👕 **きせかえ**: 設定からシャツの色を変更（グレー / レッド / オリーブ）
- 🌆 **ふんいき**: 背景のあかりを変更（よる / ゆうぐれ / しんや）
...
ヤメにゃんの画像はユーザー制作のイラストです（`assets/images/yamenyan*.png`）。

- **きせかえ**はシャツ部分だけを色変換した3種の画像を切り替え
- **ふんいき**は `ColorFiltered` のカラーマトリクスで照明を変化（アセット追加なし）
- **気分（Mood）** はセリフ・状態バッジ・重ね描きエフェクト（きらきら / ほっぺ / あせ / なみだ）で表現

これらは `lib/widgets/character_view.dart` が担当します。
...
```
assets/
  images/yamenyan.png          ヤメにゃんのイラスト（グレー）
  images/yamenyan_wine.png     きせかえ: レッド
  images/yamenyan_olive.png    きせかえ: オリーブ
lib/
